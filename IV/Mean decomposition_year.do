...
collapse (mean) p_count ntotal ptotal transp inst medimae l_distk p_iv ktv turnos n delta total URR hemo n_capacnum s_obs privado indep publico ///
pac_var s share_iv_0 share_iv_1 count_iv_0 count_iv_1 iZCAIMAE habilitados_def, ///
 by(ZCAIMAE anio_solicitud)

sort ZCAIMAE anio_solicitud
by ZCAIMAE : gen p_iv_lag = p_iv[_n-1]	
by ZCAIMAE : gen p_count_lag = p_count[_n-1]	
by ZCAIMAE : gen n_capacnum_lag = n_capacnum[_n-1]
by ZCAIMAE : gen n_lag = n[_n-1]	
by ZCAIMAE : gen s_lag = s[_n-1]	
by ZCAIMAE : gen ntotal_lag = ntotal[_n-1]
by ZCAIMAE : gen ptotal_lag = ptotal[_n-1]
...
esttab uij_tiene uij_notiene OLS IV using "C:\Users\julie\OneDrive\Documentos\Proyecto Tesis\MastersThesis\IV\year n_capacnum_lag.tex", ///
    cells(b(star fmt(3)) se(par fmt(3))) ///
    label nonumber mtitles(CL CL OLS 2SLS) ///
    style(tex) title("Patient-specific coefficients and mean decomposition") replace ///
	stats(N cdf1 rkf1, labels("Observations" "CD Wald F" "SW S stat.") fmt(%9.0fc %9.3fc %9.3fc)) ///
	eqlabels(" " " ") booktab
...
esttab uij_tiene uij_notiene OLS IV using "C:\Users\julie\OneDrive\Documentos\Proyecto Tesis\MastersThesis\IV\year n_capacnum_lag pub.tex", ///
    cells(b(star fmt(3)) se(par fmt(3))) ///
    label nonumber mtitles(CL CL OLS 2SLS) ///
    style(tex) title("Patient-specific coefficients and mean decomposition") replace ///
	stats(N cdf1 rkf1, labels("Observations" "CD Wald F" "SW S stat.") fmt(%9.0fc %9.3fc %9.3fc)) ///
...
esttab uij_tiene uij_notiene OLS IV using "C:\Users\julie\OneDrive\Documentos\Proyecto Tesis\MastersThesis\IV\year n_lag", ///
    cells(b(star fmt(3)) se(par fmt(3))) ///
    label nonumber mtitles(CL CL OLS 2SLS) ///
    style(tex) title("Patient-specific coefficients and mean decomposition") replace ///
	stats(N cdf1 rkf1, labels("Observations" "CD Wald F" "SW S stat.") fmt(%9.0fc %9.3fc %9.3fc)) ///
...
esttab uij_tiene uij_notiene OLS IV using "C:\Users\julie\OneDrive\Documentos\Proyecto Tesis\MastersThesis\IV\year n_lag pub.tex", ///
    cells(b(star fmt(3)) se(par fmt(3))) ///
    label nonumber mtitles(CL CL OLS 2SLS) ///
    style(tex) title("Patient-specific coefficients and mean decomposition") replace ///
	stats(N cdf1 rkf1, labels("Observations" "CD Wald F" "SW S stat.") fmt(%9.0fc %9.3fc %9.3fc)) ///
