...
gen p_count_notiene=p_iv*total if tiene_imae==0
gen p_count_tiene=p_iv*total if tiene_imae==1

*collapse (mean) p_iv ktv turnos n delta total URR hemo n_capacnum s_obs privado indep publico ///
*pac_var s share_iv_0 share_iv_1 count_iv_0 count_iv_1 iZCAIMAE habilitados_def ///
*p_count p_count_notiene p_count_tiene, ///
* by(ZCAIMAE anio_solicitud)

gen n_capacnum2=n_capacnum*n_capacnum
gen cong=n_capacnum/habilitados_def
...
by ZCAIMAE tiene_imae: gen n_capacnum2_lag = n_capacnum2[_n-1]	
by ZCAIMAE tiene_imae: gen cong_lag = cong[_n-1]	
...
eststo OLS: reg delta URR ntotal_lag indep privado turnos, nocons

eststo IV: ivreg2 delta URR indep privado turnos (ntotal_lag = p_count_lag), nocons  ///
ffirst savefirst r savefprefix(st1)
qui estadd scalar cdf1 =  `e(cdf)': IV
qui estadd scalar sstat1 = `e(sstat)': IV


esttab uij_tiene uij_notiene OLS IV using "iv_reg n_lag turnos.tex", ///
...
eststo OLS: reg delta URR ntotal_lag habilitados_def turnos, nocons

eststo IV: ivreg2 delta URR habilitados_def turnos (ntotal_lag = ptotal_lag), nocons  ///
ffirst savefirst r savefprefix(st1)
qui estadd scalar cdf1 =  `e(cdf)': IV
qui estadd scalar sstat1 = `e(sstat)': IV


esttab uij_tiene uij_notiene OLS IV using "iv_reg n_lag sin turnos.tex", ///
    cells(b(star fmt(3)) se(par fmt(3))) ///
    label nonumber mtitles(CL CL OLS 2SLS) ///
    style(tex) title("Patient-specific coefficients and mean decomposition") replace ///
	stats(N cdf1 sstat1, labels("Observations" "CD Wald F" "SW S stat.") fmt(%9.0fc %9.3fc %9.3fc)) ///
