...
```
# Quiénes están en SESIONES, quiénes en INGRESOS, quienes en los dos

```{r}

sesi <- SESIONES_HD %>% mutate(sesi=1) %>% select(CAPACNUM, sesi, PMD_ANIO)
ingr <- INGRESOS %>% mutate(ingr=1) %>% select(CAPACNUM, ingr, anio_solicitud)

ambos <- full_join(sesi, ingr, by="CAPACNUM") %>% 
  mutate(sesi=if_else(is.na(sesi), 0, sesi),
         ingr=if_else(is.na(ingr), 0, ingr),
         ambos=if_else(sesi==1 & ingr==1, 1, 0),
         tipo=case_when(sesi==1 & ingr==1 ~ "ambos",
                        sesi==1 & ingr==0 ~ "sesi",
                        sesi==0 & ingr==1 ~ "ingr"))
...
Logit_INGRESOS %>% 
  filter(
    depto=="01",
         ZCAIMAE!="SENNIAD HEMO",
         n_centro>1) %>% 
  group_by(tipo_imae, fecha3) %>% 
  summarize(n_centro=mean(n_centro, na.rm = T),
            cap_op=mean(cap_op, na.rm = T)) %>% 
  pivot_longer(cols=c("cap_op", "n_centro"), values_to = "values", names_to = "indicator") %>% 
ggplot(aes(x=fecha3, y=values, color=indicator)) + geom_line() + facet_wrap( ~ tipo_imae)

MENSUALES_HD  %>% 
  left_join(PACIENTES, by="CAPACNUM") %>%
  left_join(tipo_imae, by=c("ZPMD_IMAE"="ZCAIMAE")) %>%
  mutate(tipo_imae2=case_when(tipo_imae=="INDEPENDIENTE" ~ "Indep",
                              tipo_imae=="PRIVADO" ~ "Priv Ins",
                              tipo_imae=="PUBLICO" ~ "Pub Ins")) %>% 
  left_join(IMAE_num, by=c("ZPMD_IMAE"="ZCAIMAE"))  %>% 
  ...
  summarize(edad=mean(edad, na.rm = T),
           ) %>% 
  filter(fecha_mensual>"2004-03-01") %>% 
#%>% pivot_longer(cols=c("cap_op", "edad"), values_to = "values", names_to = "indicator") %>% 
ggplot(aes(x=fecha_mensual, y=edad)) + geom_line() + 
  theme(
        legend.position = "bottom",
    panel.grid.minor = element_blank(),  # Remove minor gridlines
    strip.background = element_blank()  # Remove background from facet titles
  ...
  scale_color_viridis_d(begin=0.5, end=0.8) + 
  labs(title="Edad de pacientes en hemodiálisis 2004-2016",
       y="Edad promedio", x="Año") 
