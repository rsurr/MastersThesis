...
tabla_pacientes <- pacientes %>% 
  ...
  summarize(
  "# patients" = n(),
   "Age at start" = mean(edad_min, na.rm = TRUE),
   "Age at start | Dead" = mean(edad_min[dead==1], na.rm = TRUE),
   "Age at death" = mean(edad_max[dead==1], na.rm = TRUE),
   "Months on dialysis" = mean(meses, na.rm = TRUE),
   "Months on dialysis | Dead" = mean(meses[dead==1], na.rm = TRUE),
...
tabla_pacientes <- pacientes %>% 
  ...
  summarize(
    ...
    Corporate = paste0(round(mean(CORPORATIVO, na.rm = TRUE)*100, 2), "%"),
    Diabetic = paste0(round(mean(DDIAB, na.rm = TRUE)*100, 2), "%"),
    Cardiopathy = paste0(round(mean(DCISQ, na.rm = TRUE)*100, 2), "%"),
    Vascular = paste0(round(mean(DEVP, na.rm = TRUE)*100, 2), "%"),
    Working = paste0(round(mean(DTRABA, na.rm = TRUE)*100, 2), "%"),
    Smoking = paste0(round(mean(DFUMA, na.rm = TRUE)*100, 2), "%"),
    "Decompensated start" = paste0(round(mean(descom, na.rm = TRUE)*100, 2), "%"),
    "Coordinated start" = paste0(round(mean(coord, na.rm = TRUE)*100, 2), "%")
  ) %>% 
  pivot_longer(cols=c(everything(), -tipo_imae), names_to = "Indicator", values_to = "Value") %>% 
  pivot_wider(values_from="Value", names_from = "tipo_imae") %>% 
  mutate_if(is.numeric, round, digits=2) %>% 
...
pacmens_capacity <- pacientes_mensuales %>% 
  ...
  ungroup() %>% 
  select(ID_CAIMAE, mes_solicitud, habilitados_def, slack_def, n_capacnum, tipo_imae3, tipo_imae2, fecha3, depto, ZCAIMAE)
