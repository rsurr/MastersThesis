...
/// Compute shares
	cap drop eV
	gen eV= ///
	exp(URR*b_URR + n_obs*b_n_obs + hab_op*b_hab_op + ///
...
collapse (mean) p_* b_* arancel share n_obs dSdQ ///
...
hab_op cong_op cap_op s_obs2 (first) chain, by(ZCAIMAE anio_solicitud)
...
/// GMM BASE + INDEP
eststo GMM_INDEP: gmm ( (P*dSdQ - ({a2}+2*{a3})*share)/dSdQ - ///
{a0} - {a2}*URR - {a3}*URR2 - {a4}*indep),	///
...
gen cost= b_a0 + b_a2*URR + b_a3*URR2 + b_a4*indep
...
collapse (mean) a_constant P dSdQ b_constant b_URR b_a0 b_a2 b_a3 URR share indep s_obs2 n_obs b_n_obs, by(ZCAIMAE)

egen n_obs_total = sum(n_obs)


export delimited using "C:\Users\julie\OneDrive\Documentos\Proyecto Tesis\MastersThesis\Cost\constantes n_obs.csv", replace
